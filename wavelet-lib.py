...
# Reads all files from (adreess)and returns data in files as datafram columns #
...
def retrieve_csv_files_in_folder_as_dataframe(down_sample, down_spl_val):
    ...
    for filename in glob.glob(os.path.join(addr, "*.csv")):
        my_file = open(filename)
        data = pd.read_csv(my_file, header=4, sep=r',',
...
# x: numpy ndarray reprensenting x axis,
# widths: numpy ndarray reprensenting wavelet transform scale range ***** #
...
# ********** if two_plots is False, plots dat against x.
# If two_plots is True, plots dat and dat2 against x ********* #
...
# Normalizes values in arr btw 0 & 1 by dividing them by the highst value in arr
def normalize(arr):
    maxim = max(arr)
...
# **** returns windowed and normalized time series, time shift for
# time accurate plotting, length of processed ts **** #
...
# ******* returns the highest value in a row of a nd_array passed by
...
'''
# locates first and second echoes in time and scale by returning scale_echo1, 
time_echo1, scale_echo2, time_echo2 #
# ***** returns "not found" if echo was not found ****** #
 ...
 p :    integer representing an estimate of the latest possible time(see below) 
     of the occurrence of the 1st echo
 echo2_min_pos : integer representing time(see below) of earliest possible 
                 occurrence of 2nd echo
 echo2_max_pos : integer representing time(see below) of latest possible 
                 occurrence of 2nd echo time expressed in number of samples in 
...
def locate_echoes(arr, p, echo2_max_pos):
    ...
    ar = arr[:, 0:p]
    # find highest value in each row of ar and store it in b
    b = np.apply_along_axis(get_highest_val, 1, ar)
    coeffs1 = []
    coeffs2 = []
...
# ***** selects the frequencies needed in the wavelet transform ****** #
 ...
 freq1: highest frequency needed, its default value is 1 if functions
        acts as a highpass filter **** #
...
'''
# Remove selected frequncies from time series passed as columns of a dataframe #
    Saves processed time series as a csv in selected folder. 
    Saved folder name is new_dtfrm.csv 
# dtfrm: dataframe whose columns are the time series to be processed
# band_pass: bool. If True, a band pass filtering will be performed on time series
# high_pass: boollean. If True, a high pass filtering will be performed on time series
# low_freq_limit :lowest frequency to keep in processed signal
# high_freq_limit: highest frequency to keep in processed signal. 
                    Default value is 1 if high pass filtering
...
def remove_frequencies_and_save_to_csv(dtfrm, band_pass, high_pass,
    ...
    while i < column:
        ...
        wave, scales, freqs, coi, fft, fftfreqs = wavelet.cwt(y, delta_t,
                                                              J=width-1, wavelet=u'mexicanhat')
        wave = select_frequencies(wave, width, band_pass, high_pass,
                                  delta_t, low_freq_limit, high_freq_limit)
...
##### run interpolate
# path = r"/media/kacao/479E26136B58509D/Titan_AES/Python-script/Me02-H100_180810/"
# final_log_path  = path + 'Me02-H100_180813_raw_sorted.csv'
#
# with open(final_log_path) as outfile:
#     table_sorted = pd.read_csv(outfile, sep=',', error_bad_lines=False)
# outfile.close()
#
# file_name = pd.DataFrame()
#
...
#             y.append(float(num))
#
#     interp_arr      = (interp(y, r=10, l=4, alpha=0.5))
#     interp_result   = pd.DataFrame(interp_arr)
#     file_name       = pd.concat([file_name, interp_result], axis=1, ignore_index=True)
#
#     my_file.close()
# table_sorted = pd.concat( [table_sorted, file_name], axis = 1)
# table_sorted.to_csv(path + 'interpl_sorted_logs.csv')
